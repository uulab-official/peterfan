...
**베타 — v1.26.73.** 활발히 개발 중이며, 아래 표는 실제로 출시된 기능을 그대로 반영합니다:
...
| **macOS 온도 & 팬 RPM** | ✅ 실측치 — M3 Pro/Max CPU 대표 온도는 SMC `TVD0`/`TCMb`/`Tf16`/`Tf26`/`Tf36` hotspot, `TV*` aggregate, `TCDX` summary 중 더 높은 신뢰 후보를 사용하고, 없을 때 `Te*`/`Tf*` live core 평균 fallback, IOHID `tdie`는 fallback |
