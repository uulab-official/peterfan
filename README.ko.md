...
**베타 — v1.26.76.** 활발히 개발 중이며, 아래 표는 실제로 출시된 기능을 그대로 반영합니다:
...
| **macOS 온도 & 팬 RPM** | ✅ 실측치 — M3 Pro/Max CPU 대표 온도는 Performance Core 평균을 우선 사용하는 `CPU Core Average` 기준. `TVD0`/`TCMb` hotspot은 `CPU Hottest`로 분리하고, core 키가 없을 때만 `TV*` aggregate/`TCDX` summary/IOHID `tdie` fallback 사용. 메뉴바 `전체 센서`와 `peterfan temps --all`은 원시 SMC/IOHID 온도 센서를 모두 표시 |
