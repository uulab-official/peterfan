...
**베타 — v1.26.81.** 활발히 개발 중이며, 아래 표는 실제로 출시된 기능을 그대로 반영합니다:
...
| **macOS 온도 & 팬 RPM** | ✅ 실측치 — M3 Pro/Max CPU 대표 온도는 메뉴바 우클릭 `CPU 온도 기준`에서 live P-core 평균, IOHID `tdie`, `TCDX` summary, `TV0s`/`TV1s`/`TVsa`/`TVss` SMC aggregate, hottest 중 선택 가능. 각 후보는 별도 행/진단값으로 노출해서 iStat/Macs Fan Control과 비교할 때 어떤 센서 계열이 다른지 바로 확인 가능. 메뉴바 `전체 센서`와 `peterfan temps --all`은 원시 SMC/IOHID 온도 센서를 모두 표시 |
