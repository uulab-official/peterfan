...
**베타 — v1.26.68.** 활발히 개발 중이며, 아래 표는 실제로 출시된 기능을 그대로 반영합니다:
...
| **macOS 온도 & 팬 RPM** | ✅ 실측치 — M3 Pro/Max CPU 평균은 SMC `TCDX` summary 키 우선, 없을 때 `Te*`/`Tf*` live core 평균 fallback, `TV*` aggregate는 `doctor` 비교 진단값, IOHID `tdie`는 fallback |
