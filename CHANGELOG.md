...
- 새, 오리, 양, 물고기, 슬라임, 달리는 사람, 푸시업, 궤도, 회전초밥,
  도시, 소시지, 파티 러너를 추가해 전체 선택지를 20종으로 확장했습니다.
- 각 러너는 독립적인 8프레임 실루엣을 사용합니다. 동물의 보행뿐 아니라
  날갯짓, 물결, 운동, 공전, 컨베이어, 연기, 군무처럼 종류에 맞는 움직임이
  CPU 부하에 따라 더 빠르고 크게 재생됩니다.

### Changed

- 20개 버튼이 설정 화면을 길게 밀어내지 않도록 캐릭터 선택을 compact
  메뉴로 바꿨습니다. 영어·한국어 이름, 우클릭 메뉴, 설정 저장은 같은
  `RunnerCharacter::ALL` 목록을 사용합니다.
- CLI는 `--character <name>`과 20개 선택지를 안내하며 `runner`, `push-up`,
  `reactor`, `hotdog` 같은 읽기 쉬운 별칭도 지원합니다.

### Fixed

- v1.27.73 설정 화면에 펭귄·공룡·로봇·유령 버튼이 보이지만 JavaScript
  허용 목록이 이전 4종으로 남아 클릭이 무시될 수 있던 문제를
  수정했습니다. UI 허용 목록을 20종 단일 배열로 통합했습니다.

### Tests

- 20종의 실루엣이 모두 서로 다르고, 각 캐릭터에 중복 없는 8개 프레임이
  있으며 메뉴바 폭을 충분히 채우는지 검사합니다.
- 설정 메뉴의 20개 값, 한국어 이름, JavaScript 허용 목록, CLI 도움말과
  설정 파싱을 함께 검증합니다.

## [1.27.73] — 여덟 가지 CPU 러너

### Added

- 기존 고양이·강아지·토끼·여우에 펭귄, 공룡, 로봇, 유령 러너를
  추가했습니다. 펭귄은 날개와 두 발, 공룡은 긴 꼬리와 큰 뒷다리,
  로봇은 각진 몸체와 관절, 유령은 떠다니는 꼬리와 팔이 각각 독립적인
  8프레임 동작으로 움직입니다.
- 작은 메뉴바 크기에서도 로봇과 유령의 얼굴이 뭉치지 않도록 투명
  눈·표정 디테일을 추가했습니다.

### Changed

- 설정의 캐릭터 선택 영역을 8개의 좁은 버튼으로 압축하지 않고 2열
  그리드로 정리해 영어와 한국어 이름이 모두 안정적으로 보이게 했습니다.
- 새 러너들도 기존 CPU 반응 곡선을 공유해 부하가 높아질수록 동작 폭과
  프레임 속도가 함께 증가하며, 선택은 즉시 반영되고 다음 실행에도
  유지됩니다.

### Tests

- 여덟 캐릭터의 실루엣이 서로 다르고, 각 캐릭터가 중복 없는 8개 자세를
  가지며, 메뉴바 슬롯을 충분히 채우는지 검증합니다.
- 영어·한국어 설정 화면에 여덟 선택지가 모두 노출되는지 검사하고 전체
  스프라이트 시트로 실제 렌더링을 확인했습니다.
...
- 러너의 8개 키포즈마다 몸통 압축·신장·기울기, 머리 상하·전후 이동,
  꼬리 반동, 앞·뒤 다리 보폭을 함께 바꾸도록 달리기 사이클을 다시
  구성했습니다. 빠르게 재생할 때 발만 깜빡이던 움직임을 실제
  도약·공중·착지 동작으로 개선했습니다.
- CPU 프레임 간격을 `약 420–60ms`의 7배 이상 체감 범위로 조정했습니다.
  CPU가 높아질수록 cubic 곡선으로 빠르게 가속하며, 상태 아이콘 합성이
  자기 자신을 부하로 만들지 않도록 고부하에서도 약 17fps로 제한합니다.
- macOS 러너 타이머를 전체 Tao 이벤트 루프에서 분리했습니다. AppKit 공통
  RunLoop가 캐시된 이미지만 교체하므로 센서·팝오버·팬 제어 이벤트는 러너
  프레임 속도와 독립적으로 처리됩니다.
- CPU 부하가 높을수록 보폭과 몸통 바운스 진폭도 커져 속도뿐 아니라
  움직임 자체가 더 역동적으로 보입니다.

### Tests

- 고부하 러너의 키포즈 변화 픽셀이 저부하보다 많고 전력 질주 자세
  사이에 300픽셀 이상의 실루엣 차이가 생기는지 검증합니다.

## [1.27.70] — 선명하고 빠른 CPU 러너

### Changed

- 메뉴바 러너를 정사각형 네 다리 렌더링에서 가로형 `48×32` 독립
  실루엣으로 다시 디자인했습니다. 고양이·강아지·토끼·여우의 머리와
  꼬리 형태를 구분하고 8개 접지·공중 자세가 실제 달리기로 읽히도록
  보폭을 정리했습니다.
- macOS 러너를 네이티브 템플릿 이미지로 표시해 라이트·다크 메뉴바에서
  시스템 명암을 따르고, 몸통 전체가 흐릿해지던 타원 안티앨리어싱 계산을
  가장자리 전용으로 수정했습니다.
- CPU 애니메이션 범위를 `780–55ms`로 넓히고 반응 곡선을 강화했습니다.
  CPU 45%에서는 약 176ms, 95%에서는 약 55ms로 움직이며 부하 상승은
  한 샘플 안에 82% 반영합니다.

### Tests

- 네 캐릭터의 가로형 실루엣, 8개 고유 자세, 접지 변화, 저부하 대비
  고부하 10배 이상의 프레임 속도 차이를 회귀 테스트로 고정했습니다.

## [1.27.69] — 팬 제어 설치 직후 활성화

### Fixed

- LaunchDaemon 등록 뒤 0.8초에 한 번만 확인하던 설치 흐름을 최대 10초 동안
  호환 버전과 IPC 응답을 함께 확인하도록 바꿨습니다. 느린 Mac에서도 설치
  성공을 너무 일찍 판정하지 않습니다.
- 설치 성공·실패·취소가 끝나면 이벤트 루프를 즉시 깨우고 데몬 캐시를 다시
  읽어, 팬 제어 버튼이 다음 주기나 앱 재실행을 기다리지 않습니다.
- 설치 버튼의 진행 상태를 고정 15초 타이머 대신 네이티브 완료 리비전과
  연결했습니다. 설치가 끝나는 즉시 팬 프로필과 개별 팬 제어가 활성화되고,
  실패나 취소 때도 바로 다시 시도할 수 있습니다.

## [1.27.68] — 메뉴바 숫자 표시 전환 복구

### Fixed

- 실행 중 `러너` 또는 `둘 다`에서 `숫자`로 바꾸면 아이콘 캐시가 실제 이미지보다 먼저 `숨김` 상태가 되어 러너를 제거하지 못하던 문제를 수정했습니다. 남은 아이콘이 숫자 전용 폭을 차지해 온도 숫자가 보이지 않던 원인이었습니다.
- macOS 상태바 버튼의 이미지 위치를 표시 방식별 `NoImage`, `ImageOnly`, `ImageLeft`로 명시해 숫자·러너·둘 다 전환이 즉시 동일하게 렌더링되도록 했습니다.
...
- 메뉴바 CPU 러너에 고양이, 강아지, 토끼, 여우 캐릭터를 추가했습니다. 설정 화면과 우클릭 메뉴에서 재시작 없이 선택할 수 있고 선택값은 다음 실행에도 유지됩니다.
- 네 캐릭터 모두 CPU 사용률에 따라 110–900ms 범위로 속도가 바뀌는 8프레임 달리기 애니메이션과 고정 폭 클릭 영역을 공유합니다.

### Changed

- 메뉴바 표시 방식을 `숫자/고양이/둘 다`에서 캐릭터 종류와 독립적인 `온도/러너/둘 다` 개념으로 정리했습니다.
- 앱 번들, Cargo 패키지, 라이선스와 랜딩 페이지의 제품 주체 표기를 UULab으로 통일했습니다.

### Fixed

- 비활성화된 저장소 Pages 설정 때문에 랜딩 페이지 배포가 `configure-pages` 단계에서 404로 실패하던 문제를 해결했습니다. 저장소를 GitHub Actions 배포 방식으로 활성화하고 워크플로에 자동 활성화와 정적 사이트 검증을 추가했습니다.
- macOS의 손쉬운 사용 `동작 줄이기`가 켜져 있으면 메뉴바 고양이를 정지 프레임으로 전환하고, 설정 화면에도 이 상태를 표시합니다.
- WebView 준비 재시도가 이미 성공한 준비 신호를 다시 보내 초기 데이터 적용과 로그가 중복되던 문제를 제거했습니다.
- 실제 화면에서는 사용되지 않던 compact/expanded 로컬 상태와 이를 기능이 있는 것처럼 통과시키던 문자열 테스트를 정리했습니다.
- 같은 갱신 주기에 메뉴바 온도와 전체 센서 목록이 IOHID 센서를 중복 조회하던 경로를 짧은 공유 캐시로 합쳤습니다. 팝오버가 닫혀 있을 때는 온도 표시를 3초 주기로 낮춰 상시 CPU 사용량을 줄이고, 화면을 열면 다시 2초 주기로 갱신합니다.
- macOS 메뉴바 앱을 기본 6시간 감시하며 평균·최대 CPU, RSS 상한·증가량, 스레드 증가, 프로세스 생존을 CSV로 검증하는 soak 테스트를 추가했습니다. 기존 PID를 지정하면 별도 메뉴바 앱을 띄우지 않습니다.

## [1.27.63] — 네이티브 알림 규칙

### Added

- 설정에 CPU 코어 평균 온도 경고, 팬 쓰기·RPM 검증 실패, 새 앱 업데이트 알림 토글을 추가했습니다. 계정이나 외부 서비스 없이 macOS와 Windows의 네이티브 알림으로 동작합니다.
- CPU 온도 경고는 50–110°C에서 선택할 수 있고, 임계값 아래 3°C까지 식은 뒤에만 다시 울려 경계 구간에서 반복 알림이 발생하지 않습니다.

### Safety

- 데몬의 90°C 임계 안전 알림과 자동 팬 보호는 사용자 알림 설정과 분리해 항상 유지합니다.
- 팬 실패 알림은 앱 시작 시 누적된 과거 오류를 기준점으로만 기록하고, 이후 실제 실패 카운터가 증가할 때만 표시합니다.
...
- 설정의 업데이트 영역을 `업데이트 확인`과 `지금 업데이트`로 분리했습니다. 새 서명 릴리스와 검증 파일이 준비된 경우에만 설치 버튼이 나타납니다.
- `지금 업데이트`는 macOS와 Windows 모두 최신 릴리스를 다시 확인하고 digest, `checksums.txt`, 플랫폼별 신뢰 조건을 검증한 뒤 설치와 재실행까지 이어집니다.
- 상단 간단 메뉴와 우클릭 메뉴도 모호한 확인 동작 대신 `지금 업데이트`로 표시해 즉시 설치 동작임을 분명히 했습니다.

## [1.27.59] — Windows 실측 범위와 플랫폼 안정성

### Added

- Windows 펌웨어가 ACPI 열 영역을 제공하는 경우 `System Thermal Zone` 실측값을 표시합니다. CPU 코어 온도로 오인되지 않도록 메인보드/ACPI 출처를 함께 유지합니다.
- Windows 업데이트가 네이티브 x64 ZIP을 선택하고 GitHub digest, `checksums.txt`, 내장 CLI 버전을 검증한 뒤 기존 사용자 영역 설치를 갱신합니다.
- Windows CLI의 시작 프로그램 설치·상태·제거 명령과 프로세스 종료 버튼을 실제 Windows 동작에 연결했습니다.

### Fixed

- 실제 하드웨어가 온도와 팬을 모두 제공하지 않을 때 CLI가 mock 센서로 바꾸던 문제를 제거했습니다. 부분 지원과 완전 비지원 모두 빈 값 또는 실측값 그대로 표시합니다.
- 온도 읽기에 실패한 상태에서 직접 팬 프로파일이나 커브를 0°C 기준으로 계산해 적용할 수 있던 안전 문제를 차단했습니다.
- 배터리 감지가 시작 시 한 번 실패하면 앱을 다시 실행할 때까지 나타나지 않던 문제와, 다중 디스크에서 시스템 볼륨 대신 임의 볼륨을 고르던 문제를 보완했습니다.
- 디스크·네트워크·배터리 열거를 해당 화면의 첫 느린 갱신까지 미뤄 상태바 앱의 초기 실행 지연과 불필요한 가상 볼륨 조회를 줄였습니다.
- GitHub 토큰이 제공된 자동화 환경에서는 인증된 릴리스 API 요청을 사용해 공유 CI 주소의 요청 제한 때문에 업데이트 검증이 불안정해지지 않도록 했습니다.
- Windows 콜드 WebView2 시작 검사는 준비되는 즉시 끝내되 느린 CI 머신의 여유 시간을 늘리고, 실패 시 앱 진단 로그를 함께 남깁니다.
- 팬 센서가 없는 Windows에서 온도도 계속 감시된다고 안내하던 문구와, 배터리가 없는 데스크톱을 무조건 점검 필요로 표시하던 상태를 바로잡았습니다.

## [1.27.58] — 자연스러운 메뉴바 고양이 질주

### Changed

- 메뉴바 고양이를 8프레임 질주 사이클로 다시 그려 앞·뒷다리의 교차 접지, 도약 중 다리 접기, 착지 시 몸통 신장이 자연스럽게 이어지도록 했습니다.
- 작은 메뉴바에서도 발 동작이 읽히도록 다리를 두 마디 실루엣으로 보강하고 아이콘 표시 크기를 20pt로 조정했습니다.
- CPU 사용률이 오르면 즉시 빨라지고 내려갈 때는 부드럽게 감속하는 기존 반응성은 유지하면서 모든 프레임을 시작 시 캐시해 애니메이션 중 그리기 비용을 만들지 않습니다.
- GitHub Release는 macOS와 Windows 자산이 모두 검증된 뒤에만 공개되며, 로컬 macOS 릴리스도 Windows Actions 완료와 ZIP 첨부를 기다립니다.

### Added

- Windows 설치기가 Microsoft Edge WebView2 Runtime을 확인하고 누락된 노트북에서만 Microsoft 서명 Evergreen 부트스트래퍼를 실행합니다.
- Windows 릴리스 검증에 실제 per-user 설치, Start 메뉴 바로가기, 시작 프로그램 레지스트리, 실측 CPU/메모리, 트레이/WebView 준비, 제거 수명주기를 추가했습니다.

### Tests

- 8개 자세가 모두 서로 다르고 접지·공중 프레임의 발 위치가 실제로 달라지는 회귀 테스트와 확대 스프라이트 시트 시각 QA를 추가했습니다.
- `windows-2022` 네이티브 러너에서 원본 바이너리와 설치된 ZIP을 각각 실행하고, 단일 인스턴스와 WebView 팝오버 초기화를 검증합니다.

## [1.27.57] — 팬 감지와 초기 화면 안정화

### Fixed

- 앱 시작 직후부터 팬과 데몬 상태를 백그라운드에서 미리 읽어 첫 팬 화면이 잠시 `팬 없음`으로 보이던 문제를 수정했습니다.
- 일시적인 빈 팬 응답은 세 번 연속 확인되기 전까지 마지막 정상 팬 목록을 유지해 팬 카드가 나타났다 사라지지 않게 했습니다.
- 잠자기 복귀 시 마지막 팬 정체성을 유지하되 새 샘플 전에는 제어를 잠가 잘못된 `팬 없음` 상태와 오래된 제어를 모두 방지합니다.
- 상태 및 시스템 로딩 표시를 오버레이로 바꿔 데이터가 도착할 때 아래 콘텐츠가 위아래로 움직이지 않게 했습니다.
- 데몬 IPC 조회를 UI 이벤트 루프 밖의 백그라운드 작업으로 옮겨 팬 화면 진입 시 멈춤 가능성을 줄였습니다.

## [1.27.56] — CPU 러너와 시스템 관측 범위 개선

### Added

- 설정에서 메뉴 막대 표시를 숫자, CPU 고양이, 둘 다 중 바로 선택하고 현재 러너 페이스를 확인할 수 있습니다.
- 시스템 화면에 로드 평균, 소비 전력, 합산 네트워크 속도, 가동 시간 빠른 정보를 추가했습니다.
- iStat Menus와 RunCat을 기준으로 현재 지원 범위와 다음 기능을 구분한 제품 패리티 문서를 추가했습니다.

### Changed

- CPU 고양이를 4프레임에서 6프레임으로 늘리고 고부하에서는 더 빠르게 달리도록 속도 범위를 넓혔습니다.
- CPU 부하 상승에는 즉시 반응하고 하락에는 부드럽게 감속해 짧은 샘플 변동으로 캐릭터가 버벅이지 않도록 했습니다.

## [1.27.55] — 한눈에 읽히는 상태와 팬 모드 예측성 개선

### Added

- CPU 평균 온도, CPU 사용량, 팬 RPM, 안전 복귀 상태만으로 판단하는 종합 상태 행을 추가했습니다.
- 팬 프로파일마다 소음·냉각 성향 설명과 실제 기본 커브 기반 미리보기를 추가했습니다.
- 기획, 디자인, 접근성, 배포 신뢰를 검증하는 gstack 10점 제품 체크리스트를 추가했습니다.

### Changed

- 업데이트 화면에서 설치된 앱과 최신 서명 릴리스를 구분하고, 공개 릴리스보다 앞선 개발 빌드를 명확히 표시합니다.
- 센서 목록, 차트 범위, 팬 모드, 프로세스 정렬에 키보드 및 스크린리더 상태를 추가했습니다.
- 작은 보조 글자와 오른쪽 탐색 버튼의 크기를 높이고 키보드 포커스 시 숨겨진 프로세스 버튼이 보이도록 개선했습니다.

## [1.27.54] — 팬 상태 피드백과 테마 색상 완성도 개선

### Changed

- 팬 모드를 적용하는 동안 세그먼트 하단에 레이아웃을 흔들지 않는 진행 표시를 추가했습니다.
- 적용 성공과 실패를 활성 모드와 상태 문구에서 각각 초록색과 빨간색으로 짧게 확인할 수 있습니다.
- CPU, 메모리, 온도, 디스크, 네트워크 차트와 팬 커브가 현재 다크·라이트 테마의 색상 토큰을 사용하도록 통합했습니다.
- 버튼을 누를 때 작은 축소 피드백을 추가하고 시스템의 모션 축소 설정에서는 모든 진행 효과를 정적으로 표시합니다.
- 강조·성공·경고·오류 배경색을 공통 토큰으로 정리해 화면마다 미묘하게 달랐던 상태 색을 맞췄습니다.

## [1.27.53] — 팬·설정·시스템 화면 제품 디자인 개선

### Changed

- 팬 프로파일을 하나의 컴팩트한 세그먼트 컨트롤로 묶고 현재 자동 모드가 가장 먼저 읽히도록 정리했습니다.
- 커브 입력, 안전 온도, 임계 온도는 카드 대신 얇은 진단 행으로 낮춰 팬 RPM과 실제 조작 영역에 시선이 먼저 가도록 했습니다.
- 설정 화면은 일반과 업데이트, 시스템 화면은 하드웨어 지표로 내부 제목과 역할을 명확히 분리했습니다.
- 팝오버 HTML에 실제 언어 속성을 추가하고 한글·영문 내부 제목을 함께 검증하도록 보강했습니다.
- 팝오버 QA 이미지를 상태·팬·설정·시스템 네 화면과 다크·라이트, 한글·영문 조합으로 확장했습니다.

## [1.27.52] — 팝오버 시각 체계와 전환 안정성 개선

### Changed

- CPU, 메모리, CPU 평균 온도, 팬 평균 RPM을 한 줄의 실시간 요약으로 압축하고 상세 지표는 차분한 전체 너비 섹션으로 정리했습니다.
- 상태·팬·설정·시스템 화면에 같은 높이의 헤더를 유지해 탭 전환 시 콘텐츠가 위아래로 움직이지 않도록 했습니다.
- 오른쪽 탐색을 50px 아이콘 레일로 단순화하고 다크·라이트 모드의 표면, 대비, 상태 색을 하나의 시각 체계로 맞췄습니다.
- 키보드 포커스 표시와 시스템의 모션 축소 설정을 지원하도록 상호작용 상태를 보강했습니다.
- 다크·라이트 및 영문·한글 팝오버 QA 이미지를 실제 440px 비율과 전체 센서 목록 기준으로 갱신했습니다.
...
- 상태 화면 상단에 PeterFan 브랜드와 2분·1시간·1일 기간 선택을 한 영역으로 정돈했습니다.
- CPU, 메모리, CPU 평균 온도, 팬 평균 RPM을 큰 숫자와 얇은 상태 막대가 있는 2×2 지표로 재구성했습니다.
- 상세 지표와 오른쪽 탐색의 표면, 간격, 활성 상태를 다크·라이트 모드에서 일관되게 다듬었습니다.
- 팝오버 시각 QA 렌더러와 다크·라이트, 영문·한글 기준 이미지를 새 디자인에 맞췄습니다.

## [1.27.48] — 상태 화면 요약 지표 개선

### Added

- 상태 화면 상단에 CPU 부하, CPU 평균 온도, 팬 평균 RPM을 한눈에 보여주는 요약 영역을 추가했습니다.
- 팬 센서가 없거나 아직 읽는 중인 경우 숫자를 추측하지 않고 명확한 상태를 표시합니다.
- 팝오버 시각 QA 이미지와 렌더러를 최신 요약 영역까지 반영했습니다.
...
- 시스템 화면 첫 진입 시 빈 숫자만 보이던 문제를 개선해 실제 느린 지표를 읽는 동안 명확한 상태를 표시합니다.
- 저장공간·프로세스 데이터가 준비되면 로딩 상태가 자동으로 사라지고 기존 화면으로 자연스럽게 전환됩니다.

## [1.27.44] — 설정과 시스템 지표 화면 분리

### Changed

- 설정 화면은 시작 실행, 팬 제어 안전 상태, 업데이트만 보여 실제 설정 작업에 집중할 수 있게 했습니다.
- 저장공간, 배터리, 네트워크, 프로세스는 새 시스템 화면으로 분리하고 화면 전환 시 해당 지표만 갱신합니다.
- 기존 `더보기` 화면 상태를 새 시스템 화면으로 자동 변환해 저장된 화면 설정도 그대로 이어집니다.

## [1.27.43] — 지연 로딩 복구 흐름 개선

### Fixed

- **첫 데이터 지연 시 멈춘 것처럼 보이는 문제** — 일정 시간 이상 센서 조회가
  걸리면 상태 안내와 재시도 버튼을 표시합니다.
- **재시도 경로의 UI 정지 방지** — 재시도는 네이티브 갱신 루프만 깨우고 무거운
  센서 조회는 기존 백그라운드 작업에서 처리하도록 연결했습니다.

### Changed

- 센서 데이터가 도착하면 재시도 UI가 자동으로 사라지고 정상 상태로 돌아갑니다.

## [1.27.42] — 초기 상호작용과 센서 상태 개선

### Fixed

- **초기 클릭 경합** — 첫 센서 데이터가 도착하기 전에 팬 프로필, 진단,
  업데이트, 시작 설정 버튼이 잘못 실행될 수 있던 경로를 막았습니다.
- **온도 섹션 소실** — CPU 온도 센서를 일시적으로 읽지 못해도 온도 영역을
  유지하고 현재 상태를 안내합니다.
- **오래된 온도 차트 잔상** — 센서 조회가 실패하거나 비어 있을 때 이전 차트가
  남아 있지 않도록 초기화합니다.

### Changed

- 제어 버튼은 센서/권한 상태가 확인된 뒤에만 활성화됩니다.

## [1.27.41] — 초기 화면 및 하드웨어 구성 보강

### Fixed

- **하드웨어 감지 카드가 숨겨져 있던 문제** — 설정 화면에서 팬·배터리·네트워크
  감지 상태를 실제로 확인할 수 있도록 카드 표시를 연결했습니다.
- **첫 로딩이 빈 화면처럼 보이던 문제** — 센서 조회가 끝나기 전에도 현재 상태를
  알려주는 비차단 로딩 표시를 추가했습니다.

### Changed

- 센서 데이터가 도착하면 로딩 표시가 자동으로 사라지고, 기존 화면 업데이트는
  그대로 유지됩니다.

## [1.27.40] — 팬 하드웨어 상태 표시 보강

### Fixed

- **팬 상태 오표시** — 팬 센서가 없거나 읽기 전용인 Mac에서 상단에
  `준비 완료`가 표시되거나 프로필 버튼이 활성화되는 문제를 수정했습니다.
- **빈 팬 화면 안내** — 팬 없음과 읽기 전용 팬을 구분하고, RPM 모니터링과
  CPU·메모리·온도 모니터링이 계속된다는 사실을 화면 안에서 명확히 안내합니다.

### Changed

- 실제 감지된 제어 가능 팬 수가 1개 이상일 때만 팬 프로필과 최초 설정 버튼을
  활성화합니다.

## [1.27.39] — 팬 화면 안정화 및 정리

### Fixed

- **팬 화면 상단 상태가 잠깐 나타났다 사라지는 문제** — 첫 데몬 조회가
  끝나기 전 임시 설정 배너를 그렸다가 숨기던 흐름을 제거하고, 확인 중·준비
  완료·설정 필요·읽기 전용 상태를 같은 고정 헤더에서 갱신하도록 수정했습니다.

### Changed

- **팬 화면 정보 구조** — CPU 커브 입력, 안전 최고 온도, 임계값을 한 줄 요약으로
  고정하고, 팬 프로필 버튼을 3열 그리드로 정렬해 좁은 팝오버에서도 읽기 쉽게
  다듬었습니다.

## [1.27.38] — OTA 교체 순서 안정화

### Fixed

- **업데이트 직후 앱이 사라질 수 있던 경로** — 새 번들을 덮어쓰기 전에 기존
  PeterFan 프로세스가 실제로 종료됐는지 최대 10초 동안 확인합니다. 기존
  프로세스가 살아 있으면 교체하지 않고 실패 상태를 기록해 단일 실행 잠금과
  새 앱의 상태 확인이 서로 충돌하지 않도록 했습니다.

### Quality

- OTA 셸 스크립트가 기존 프로세스 종료를 기다리는지 회귀 테스트로 고정했습니다.

## [1.27.37] — 팝오버 입력 안정화

### Fixed

- **팝오버 버튼·스크롤 입력** — 사전 준비된 창을 시작 시에는 포커스하지 않되, 사용자가 메뉴바에서 열었을 때만 포커스를 부여해 WebView 입력이 정상적으로 전달되도록 수정했습니다.

### Quality

- 공개 DMG 설치 후 실제 메뉴바 팝오버의 입력 경로를 로컬 로그로 확인했습니다.

## [1.27.36] — 설정 화면 갱신 안정화

### Fixed

- **팬 제어 상태 카드 갱신** — 설정 화면의 중복 조건문 때문에 데몬·승인·RPM 검증 상태가 오래된 값으로 남을 수 있던 문제를 수정했습니다.
- **탭 전환 진단 로그** — 팝오버와 상세 창의 상태·팬·설정 탭 요청을 로컬 로그에 남겨 입력 경로를 확인할 수 있게 했습니다.

### Quality

- 설정 화면 갱신 분기 회귀 테스트를 추가했습니다.

## [1.27.35] — 절전 복귀 안정화

### Fixed
- **절전 후 오래된 센서값 표시** — 이벤트 루프가 장시간 멈춘 뒤 복귀하면
  온도·팬·데몬·차트 캐시를 폐기하고 하드웨어를 즉시 다시 읽음.
- **Universal 릴리즈 빌드의 디스크 고갈 위험** — arm64와 x86_64 Rust 중간
  산출물을 별도 임시 작업공간에서 순차 생성하고 바이너리 복사 후 즉시 정리함.

### Quality
- 절전 복귀 간격 감지와 차트 캐시 초기화 회귀 테스트 추가.
- workspace 전체 테스트 221개 통과, 무시된 하드웨어 출력 테스트 5개.
...
### Changed
- **세 화면으로 정리한 메뉴바 UI** — 상태·팬·설정만 남기고 업데이트와 시스템
  정보는 설정 안으로 통합함. 기본 화면에서는 숫자와 그래프를 먼저 보여 주고,
  데몬 진단과 전체 센서 목록은 필요할 때만 펼쳐 보도록 정리함.
- **활성 화면별 지표 갱신** — 프로세스·디스크·배터리·팬 데몬·전체 온도 센서를
  현재 화면에서 필요할 때만 읽고 직렬화함. 탭 전환은 화면을 먼저 그린 뒤 다음
  프레임에서 데이터를 반영해 클릭 직후 멈추는 느낌을 줄임.

### Fixed
- **이미 설치된 팬 데몬이 관리자 설치를 다시 띄우던 문제** — 버튼 클릭 시 데몬
  버전과 IPC 상태를 새로 확인함. 호환 데몬이 준비된 경우 아무 파일도 바꾸지 않고,
  업데이트가 필요한 실행 중 데몬은 무승인 자체 업데이트만 사용함. 공용 설치
  함수도 정상 데몬을 발견하면 즉시 종료해 CLI를 포함한 모든 설치 경로를 보호함.
- **팬 설치 실패 원인을 숨기던 오류** — AppleScript 표준 오류를 보존해 사용자
  취소와 `launchctl`·스크립트 실패를 구분함. 설치 스크립트는 고유 임시 파일,
  절대 명령 경로, plist 검증과 label 기반 기존 서비스 해제를 사용함.
- **순간적인 데몬 응답 실패를 미설치로 표시하던 문제** — 마지막 정상 상태를 짧게
  유지해 단일 IPC 실패가 설치 안내나 비활성 팬 화면으로 바뀌지 않게 함.

### Quality
- **실제 설치 상태 검증** — 로컬 `peterfand 1.27.29`가 호환·응답 상태임을
  확인하고, 이 상태에서 설치 계획이 `AlreadyReady`가 되어 추가 관리자 승인을
  요청하지 않는 회귀 테스트를 추가함.
...
### Fixed
- **성공한 데몬 갱신이 실패로 보이던 상태** — root 데몬이 새 Developer ID
  바이너리를 검증하는 동안 일반 IPC의 0.5초 제한이 먼저 끝나더라도 즉시 실패로
  기록하지 않음. 재설치 명령에는 5초 응답 시간을 사용하고, 응답 연결이 재시작으로
  끊긴 경우에도 설치된 버전과 새 IPC 연결을 최대 10초 확인해 실제 결과를 표시함.

### Quality
- **공개 DMG 실설치 회귀 검증** — `v1.27.29` 공개 DMG를 설치해 앱 단일 실행과
  `1.27.22 → 1.27.29` 데몬 무암호 교체가 실제로 완료되는 것을 확인하고, 그 과정에서
  발견한 느린 서명 검증 응답을 회귀 테스트로 고정함.

## [1.27.29] — 메뉴바 안정성 및 팬 제어 기본값 정비

### Fixed
- **다중 모니터 메뉴바 클릭 불안정** — 클릭 순간의 실제 `NSStatusItem` 위치를
  기준으로 팝오버를 배치하고, 러너 프레임이 바뀌어도 메뉴바 항목 너비가 변하지
  않게 고정함. 외장 모니터에서 두 화면 사이로 이동하거나 클릭 영역이 흔들리지 않음.
- **러너 애니메이션 버벅임** — 매 프레임 PNG 인코딩과 `NSImage` 재생성을 제거하고
  시작 시 생성한 16개 네이티브 프레임을 재사용함. CPU 부하에 따른 속도 변화는
  유지하면서 고부하 측정에서 PeterFan CPU 사용량을 약 3.5%에서 1.5%로 낮춤.
- **OTA 직후 간헐적 실행 실패** — 앱 교체 직후 LaunchServices가 일시적으로
  `-600`을 반환하면 백업으로 성급히 되돌리지 않고, 백그라운드 실행과 실제 PID
  확인을 최대 네 차례 재시도함. 롤백 경로도 같은 생존 확인을 사용함.
- **균형 모드의 과도한 팬 속도** — 70~85°C 구간의 기본 곡선을 완만하게 조정함.
  80°C에서 약 65%, 85°C에서 75%를 사용하고 90°C 안전 경계에서는 100%를 유지함.
- **새 설치가 곧바로 팬을 제어하던 동작** — 명시적 프로필이나 저장 상태가 없는
  최초 설치는 macOS 자동 제어로 시작함. 사용자가 선택한 프로필과 개별 팬 설정은
  계속 저장하고 즉시 복원함.

### Changed
- **데몬 갱신 시 반복되는 암호 입력 제거** — 한 번 설치된 호환 데몬은 서명된 앱
  번들의 새 데몬을 직접 검증·교체하고 재시작함. `1.27.22` 사용자는 앱 업데이트
  뒤 관리자 암호 없이 `1.27.29` 데몬으로 자동 갱신됨.

### Quality
- **회귀 및 정적 검사 확대** — 메뉴바 너비, CPU 기반 러너 속도, 최초 자동 모드,
  무암호 데몬 갱신, 완화된 균형 곡선, OTA 재시도·롤백을 자동 테스트로 고정함.
...
### Fixed
- **새 Mac의 동작하지 않는 팬 버튼** — 팬 하드웨어 지원 여부와 현재 앱의
  실제 쓰기 권한을 분리함. 호환 데몬이 없고 앱도 관리자 권한이 아니면 RPM은
  계속 표시하되 프로필, 개별 팬 모드, 슬라이더를 비활성화하고 1회 설정 버튼을
  표시함. 적용할 수 없는 명령이 성공 가능한 것처럼 보이지 않음.
- **무결성 검사의 성공 종료 오류** — 앱, DMG, 릴리스 또는 릴리스 디렉터리
  검사에 실패하면 사람이 읽는 출력과 JSON 출력 모두 비정상 종료 코드를
  반환함. 자동 배포가 빨간 실패 보고서를 성공으로 오인하지 않음.

### Quality
- **프로세스 단위 회귀 검사** — 존재하지 않는 앱의 무결성 검사가 일반/JSON
  모드에서 유효한 보고서를 출력하면서 반드시 실패 종료하는 스모크 검사를 추가함.
- **다중 모니터 좌표 회귀 검사** — 오른쪽 외부 화면과 위쪽 Retina 화면의
  AppKit top-left 변환을 고정해 클릭 화면 밖으로 되돌아가는 회귀를 차단함.
- **앱 전용 업데이트** — 최소 데몬 버전은 `1.27.22`로 유지함. 기존 호환
  데몬은 재설치하지 않으며 관리자 암호를 다시 요구하지 않음.

## [1.27.26] — AppKit 직접 배치와 느린 팬 적용 확인

### Fixed
- **혼합 배율 화면 사이에 뜨는 팝오버** — macOS에서 Tao의 이전 화면 배율과
  비동기 창 이동을 더 이상 거치지 않음. 클릭한 `NSScreen`을 기준으로
  `NSWindow`의 콘텐츠 크기와 top-left 위치를 AppKit 포인트 좌표에서 동기적으로
  설정하고, 예전 물리 좌표 fallback은 macOS 빌드에서 완전히 제거함.
- **느린 Mac의 팬 설정 확인** — 팬 제어 명령 뒤 하드웨어 적용 리비전을 집중
  확인하는 시간을 1.5초에서 8초로 늘리고 UI 시간 제한도 8초로 일치시킴.
  데몬이 실제 팬 쓰기와 RPM 확인을 끝내기 전에 실패처럼 보이던 상태를 제거함.

### Added
- **팬 명령 진단 로그** — 팝오버에서 받은 팬 명령과 최종 데몬 응답을
  `~/Library/Logs/PeterFan/menubar.log`에 기록해 다른 Mac에서도 클릭 전달과
  하드웨어 적용 실패를 구분할 수 있음.

### Quality
- **앱 전용 업데이트** — 최소 데몬 버전은 `1.27.22`로 유지함. 관리자 암호나
  팬 제어 데몬 재설치 없이 앱만 교체됨.

## [1.27.25] — 혼합 배율 모니터 배치와 팬 적용 확인

### Fixed
- **외장 모니터 팝오버 위치** — 1x 외장 모니터와 2x Retina 내장 화면 사이를
  오갈 때 숨겨진 창의 이전 배율로 클릭 좌표를 다시 환산해 팝오버가 두 화면
  사이에 뜨던 문제를 수정함. AppKit에서 실제 클릭 화면을 찾고 전 과정을 macOS
  논리 좌표로 배치하여 클릭한 화면 경계 안에 유지함.
- **팬 설정의 불명확한 반응** — 모드 버튼을 누른 직후 선택만 바뀌고 실제 하드웨어
  적용 여부를 알 수 없던 상태를 개선함. 데몬의 적용 리비전이 증가하고 선택 모드가
  일치해야 `적용 완료`를 표시하며, 실패와 확인 시간 초과를 별도로 안내함.

### Added
- **팬 제어 결과 요약** — 팬 화면에 현재/적용 중 모드, 곡선 입력 온도, 실제 목표
  속도를 한 줄로 표시함. 높은 온도에서 균형 모드도 빠르게 동작하는 이유와 명령
  미적용을 구분할 수 있음.

### Quality
- **혼합 DPI 회귀 검사** — 좌우 외장 화면과 Retina 화면의 배율이 달라도 팝오버가
  클릭한 화면을 벗어나지 않는 좌표 테스트를 추가함.
- **비밀번호 없는 앱 업데이트** — 최소 데몬 버전은 `1.27.22`로 유지함. 이번 버전은
  앱만 교체하며 팬 제어 재설치나 관리자 암호 입력이 필요하지 않음.

## [1.27.24] — 메뉴바 클릭 복구와 비동기 센서 읽기

### Fixed
- **다른 Mac의 메뉴바 클릭** — macOS 상태 항목의 mouse-down에서 즉시 팝오버를
  열고 이어지는 mouse-up을 중복 제거함. 메뉴바 자동 숨김이나 디스플레이 전환 중
  mouse-up이 유실돼 아이콘을 눌러도 반응이 없던 경로를 제거함.
- **센서 읽기 중 UI 정지 방지** — SMC/IOHID 온도, 전체 센서 목록, 팬 RPM 조회를
  이벤트 루프 밖에서 실행함. 하드웨어 호출이 느리거나 멈춰도 메뉴바 클릭과 버튼
  이벤트는 계속 처리됨.

### Added
- **영구 메뉴바 진단 로그** — 시작, 단일 실행 거부, 클릭 이벤트, 팝오버 위치,
  WebView 생성 실패를 `~/Library/Logs/PeterFan/menubar.log`에 순환 기록함. 오른쪽
  클릭 메뉴의 `진단 로그 열기…`로 터미널 없이 확인할 수 있음.

### Quality
- **클릭·동시성 회귀 검사** — Down/Up 클릭 중복 제거와 중복 하드웨어 작업 방지를
  자동 테스트함. 앱 업데이트만 필요하며 최소 데몬 버전은 `1.27.22`로 유지됨.
...
### Added
- **업데이트 결과 보존** — 분리 실행되는 OTA 업데이터가 설치 중, 설치 완료,
  자동 롤백, 실행 실패 상태를 Application Support의 단일 JSON 레코드에 원자적으로
  기록함. 앱이 다시 실행된 뒤에도 업데이트 화면에서 마지막 결과를 확인할 수 있음.

### Fixed
- **롤백 결과 정합성** — 새 앱의 서명·공증·실행 확인이 끝난 뒤에만 설치 완료로
  표시하고, 이전 앱 복원 성공과 복원 자체의 실패를 구분함. 기존 앱 이동 전 실패는
  설치 상태를 변경하지 않은 것으로 별도 안내함.
- **배터리 의존성 보안** — 유지보수가 중단된 `battery 0.7` 대신 호환되는 관리 포크를
  사용해 오래된 `nix 0.19` 취약 의존성을 제거함. macOS 배포물에 연결되지 않는
  타깃별 RustSec 예외는 근거와 함께 `audit.toml`에 격리함.

### Quality
- **OTA 상태 회귀 테스트** — 상태 레코드의 제한된 덮어쓰기, 업데이트 스크립트의
  성공·롤백 기록 순서, 잘못 서명된 앱 주입 후 실제 복원 결과를 자동 검증함.
- **RustSec CI 게이트** — 모든 push와 pull request에서 `cargo audit`을 실행해 새로
  유입되는 취약 의존성이 릴리스 전에 빌드를 실패시키도록 함.
