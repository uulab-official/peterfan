...
### Removed
- **CLI 라이선스 진입점** — 이미 앱과 데몬에서 제거된 체험판·라이선스 정책이
  `peterfan license` 도움말과 설정 모델에 남아 있던 문제를 정리함. PeterFan의 모든
  기능은 계정, 로그인, 체험판, 라이선스 키 없이 바로 사용할 수 있음.
- **미사용 라이선스 코드** — 제품에서 참조하지 않는 키 검증 모듈, 키 생성 도구와
  암호화 의존성을 제거해 배포물과 유지보수 범위를 줄임.

### Fixed
- **오픈소스 문서 일관성** — 한국어·영어·일본어·중국어 README, CLI 문서와 소개
  이미지 생성기의 버전·계정 정책을 현재 제품과 일치시킴.

## [1.27.16] — 팬 RPM 적용 검증

### Added
- **지연 RPM 판독** — 데몬이 팬별 현재 RPM, 목표 RPM, 허용 오차와
  `settled`/`adjusting`/`stale` 상태를 여러 제어 주기에 걸쳐 검증해 IPC와
  CLI에 노출함.
- **팬 응답 상태 UI** — 팬 카드와 설정의 Fan Control Health에서 적용됨,
  조정 중, 응답 없음 상태를 동일한 데몬 판독 기준으로 표시함.

### Fixed
- **팬 정체 안전 복귀** — 충분한 가속 시간 뒤에도 목표 방향으로 RPM이 움직이지
  않는 상태가 3회 연속 확인되면 macOS 자동 제어로 복귀하고 15초 뒤 안전하게
  재시도함. 단일 지연 샘플이나 정상적인 팬 관성은 오류로 처리하지 않음.
- **데몬 백그라운드 부하** — 자동화 규칙에 필요하지 않은 프로세스·디스크·네트워크
  스캔을 제어 주기에서 제거해 팬 모드 전환 경로의 I/O와 시스템 로그 잡음을 줄임.
- **완성도 문서 정합성** — 이미 구현된 OTA 롤백을 로드맵에 반영하고 제거된
  라이선스 화면 표현을 현재 설정·시스템 구조로 바로잡음.

## [1.27.15] — 팬 모드 적용 피드백 개선

### Fixed
- **팬 속도 비율 기준 통일** — 현재 속도를 단순히 최대 RPM으로 나누지 않고,
  제어 명령과 동일하게 팬의 최소~최대 RPM 구간을 0~100%로 환산해 균형 모드와
  수동 슬라이더가 같은 속도를 서로 다른 값으로 해석하던 문제를 수정함.
- **느린 SMC 초기화를 UI 응답과 분리** — Apple Silicon의 최초 수동 모드 잠금
  해제가 오래 걸려도 명령 접수 응답과 상태 조회가 막히지 않으며, 실제 쓰기
  완료 리비전으로 성공 여부를 계속 확인함.

### Added
- **팬별 적용 목표 표시** — 데몬이 마지막으로 성공한 팬별 목표 듀티와 적용
  시각을 IPC에 노출하고, 메뉴바에서 현재 RPM, 목표 RPM, 조정 중/적용됨 상태와
  목표 위치 마커를 함께 표시함.

## [1.27.14] — 팬 모드 즉시 적용

### Fixed
- **제어 루프 즉시 깨우기** — 자동·프로파일·고정 모드 명령이 들어오면 데몬의
  대기 시간을 즉시 중단해 다음 주기까지 기다리지 않고 팬 설정을 적용함.
- **느린 센서 스캔과 제어 분리** — 전체 SMC 센서 스캔 중 들어온 명령은 마지막
  정상 온도 캐시로 팬에 먼저 반영하고, 이전 스냅샷의 늦은 쓰기는 리비전 검사로
  폐기해 새 프로파일이 되돌아가지 않게 함.
- **실제 적용 완료 확인** — 명령 접수 상태와 하드웨어 쓰기 완료 상태를 별도
  리비전으로 노출하고, 메뉴바는 팬 쓰기가 성공한 뒤에만 진행 상태를 해제함.
- **메뉴바 입력 지연 제거** — WebView와 백그라운드 명령 완료가 이벤트 루프를
  직접 깨우며, 적용 확인 중에는 데몬 상태를 100ms 간격으로 짧게 갱신함.

### Quality
- **적용 리비전·경쟁 조건 회귀 테스트** — 팬 쓰기 전에는 요청이 미적용 상태이고
  성공한 쓰기 뒤에만 완료되는 동작, 오래된 규칙 스냅샷이 새 수동 프로파일을
  덮어쓰지 못하는 동작을 테스트로 고정함.

## [1.27.13] — 시스템 화면과 CPU 러너 반응 개선

### Changed
- **더보기를 시스템으로 재구성** — 역할이 모호했던 `더보기`를 `시스템`으로
  바꾸고 저장공간, 배터리, 네트워크, 실행 중 프로세스와 하드웨어 감지 상태를
  한 화면에 모음. `설정`에는 팬 제어 안전 상태와 시작 시 자동 실행만 남겨 두
  화면의 목적을 명확히 분리함.
- **CPU 러너 속도 차이 확대** — 메뉴바 고양이를 저부하 약 1fps, 중간 부하
  약 2.5fps, 고부하 약 7fps로 움직이는 비선형 곡선으로 변경함. CPU가 급격히
  올라가면 기존의 느린 프레임 대기를 기다리지 않고 다음 프레임을 즉시 앞당김.

### Quality
- **러너 속도 경계 테스트와 시각 QA 갱신** — 5%, 45%, 95% CPU 구간의
  프레임 간격을 회귀 테스트로 고정하고, 한글·영문/라이트·다크 QA 이미지의
  오른쪽 메뉴를 `시스템` 구조로 갱신함.

## [1.27.12] — 제어 백오프와 OTA 자동 롤백

### Added
- **프로덕션 준비 체크리스트** — 하드웨어 안전, OTA 복구, 장시간 안정성,
  센서 신뢰도, 릴리스 증거를 [`docs/PRODUCTION_CHECKLIST.md`](./docs/PRODUCTION_CHECKLIST.md)에서
  지속적으로 관리함.
- **팬 제어 재시도 상태** — 팬 쓰기 실패 후 5초부터 최대 60초까지 지수
  백오프를 적용하고 다음 재시도까지 남은 시간을 설정 화면과 IPC에 노출함.

### Fixed
- **반복 팬 쓰기 억제** — 하드웨어 쓰기 실패 상태에서 매 센서 주기마다 같은
  명령을 반복하지 않고 OS 자동 제어를 유지한 채 안전한 간격으로만 복구를 시도함.
- **OTA 실행 실패 자동 롤백** — 새 앱을 복사한 뒤 서명과 공증 티켓을 다시
  검증하고 최대 8초 동안 실행 상태를 확인함. 복사, 검증, 실행 중 하나라도
  실패하면 보관한 이전 앱을 복원하고 백그라운드로 다시 실행함.

### Quality
- **실행형 롤백 오류 주입 테스트** — 잘못 서명된 교체 앱을 updater 스크립트에
  주입해 이전 앱 파일이 실제로 복원되는지 자동 검증함.

## [1.27.11] — 팬 제어 실패 안전장치

### Added
- **팬 제어 안전 상태 표시** — 메뉴바 상태 화면과 CLI JSON에 센서 실패 횟수,
  팬 쓰기 실패 횟수, 마지막 정상 센서 시각, 최근 오류를 노출해 실제 제어 상태를
  확인할 수 있게 함.

### Fixed
- **센서 실패 시 OS 자동 복귀** — 온도 읽기 실패, 빈 센서 목록, `0°C` 또는
  `125°C`를 넘는 비정상 값이 들어오면 듀티 제어를 중단하고 모든 팬을 macOS
  자동 모드로 즉시 복귀시킴.
- **팬 쓰기 실패 감시** — 프로필, 고정 속도, 자동 모드 적용 중 하나라도 쓰기가
  실패하면 나머지 요청 상태를 유지한 채 실제 팬은 OS 자동 모드로 안전하게
  전환하고, 센서와 쓰기가 회복된 뒤에만 요청 모드를 다시 적용함.

### Quality
- **오류 주입 회귀 테스트 추가** — 센서 단절과 팬 쓰기 실패를 모의해 듀티 쓰기
  차단, OS 자동 복귀, 상태 카운터 및 IPC 노출을 자동 검증함.
...
### Changed
- **팬 프로필 선택 즉시 피드백** — 자동·저소음·균형·게임·성능·최대 버튼을
  누르면 다음 센서 주기를 기다리지 않고 선택 및 처리 중 상태를 즉시 표시하며,
  완료 전 중복 명령 전송을 막음.
- **개별 팬 자동/사용자 지정 상태 안정화** — 데몬 확인값이 도착하기 전에도
  선택 상태와 RPM 입력 영역을 유지해 화면이 자동 상태로 잠깐 되돌아가는 현상을
  제거함.
- **팬 명령 완료 후 캐시 즉시 갱신** — 2초 주기의 데몬 캐시를 명령 완료 시
  무효화하고 다음 이벤트 루프 갱신에 합쳐 불필요한 이중 렌더링 없이 결과를 반영함.

## [1.27.5] — 팬 기본 상태와 OTA 무결성 보강

### Fixed
- **기본 자동 모드가 선택되지 않던 문제 수정** — 구형 데몬이라도 읽기 가능한
  현재 모드는 UI에 반영하고, 별도 수동 고정이 없는 초기 상태에서는 팬 화면의
  `자동` 버튼이 즉시 활성화되도록 함.
- **OTA 체크섬 검증 범위 확대** — 업데이트 자산뿐 아니라 GitHub가 제공하는
  `checksums.txt` 자체의 SHA-256도 설치 전에 검증하고, 해시 접두사와 대소문자
  형식을 안전하게 정규화함.

## [1.27.4] — 상태 화면 구분선과 여백 정밀 조정

### Fixed
- **기간 탭 아래 CPU 구분선이 붙어 보이던 문제 수정** — 숨겨진 팬 커브 행까지
  `.row + .row`로 인식해 CPU 위에 선을 만들던 선택자를 제거하고, 실제로
  구분선이 필요한 메모리·온도·배터리·네트워크·프로세스 섹션만 명시함.
- **기간 탭 상하 여백 균형 조정** — `2m / 1h / 1d` 탭에 상하 10px 패딩을
  적용해 탭 아래부터 CPU 제목까지 26px의 안정적인 간격을 확보함.

## [1.27.3] — 화면 간격과 탭 응답성 개선

### Changed
- **화면별 간격 규격 통일** — 상태, 팬, 업데이트, 설정, 더보기 화면이 같은
  좌우 여백과 섹션 패딩을 사용하도록 정리하고 버튼 높이와 카드 간격을 맞춤.
- **현재 화면만 갱신하는 렌더링 경로 적용** — 매초 숨겨진 차트, 센서,
  프로세스, 팬과 설정 화면까지 다시 만들던 구조를 제거해 현재 탭에 필요한
  DOM과 캔버스만 갱신하도록 변경함.

### Fixed
- **더보기 상단의 과도한 빈 공간 제거** — 공통 패널에 강제되던 220px 최소
  높이를 없애 실제 콘텐츠 높이만 사용하게 함.
- **탭 클릭 직후 멈춤과 늦은 화면 전환 완화** — 탭 표시 상태와 전체 레이아웃을
  매 metrics tick마다 다시 적용하지 않고 실제 탭 클릭 시 한 번만 적용함.
- **팝오버 첫 클릭이 먹히지 않던 macOS 동작 수정** — 포커스를 가져오지 않는
  팝오버 WebView가 첫 마우스 이벤트를 직접 받도록 설정해 버튼을 두 번 누르지
  않아도 즉시 실행되게 함.

## [1.27.2] — 대시보드 탐색과 전환 안정화

### Changed
- **설정 화면의 진단 정보 계층 정리** — 일상적으로 확인할 데몬, 제어 경로,
  마지막 명령, 팬, 승인 상태만 먼저 보이고 센서·서명·LaunchDaemon 상세는
  `기술 정보`에서 필요할 때 펼치도록 단순화함.
- **팬 제어 안내를 하나의 진입점으로 통합** — 오래되거나 설치되지 않은
  데몬의 안내 버튼을 상단 설정 영역 하나로 모으고 버전과 승인 필요 여부를
  짧게 표시해 중복 동작과 잘리는 문구를 제거함.

### Fixed
- **선택하지 않은 상태 버튼까지 파랗게 보이던 이중 활성화 수정** — 현재
  화면의 버튼 하나에만 활성 스타일과 `aria-pressed` 상태가 적용되게 함.
- **투명 WebView 화면 전환 잔상 완화** — 팝오버와 WebView를 불투명 배경으로
  렌더링해 탭 전환 중 검은 영역이나 이전 프레임이 비치는 현상을 줄임.

## [1.27.1] — 대시보드 첫 화면 복구

### Fixed
- **대시보드 값이 모두 `—`로 남던 JavaScript 오류 수정** — Fan Control Health가
  지역 함수에 잘못 의존하던 참조를 전역 텍스트 갱신 함수로 통일해 CPU, 메모리,
  온도와 차트 payload가 첫 갱신부터 정상 렌더링되게 함.
- **메뉴바 클릭 후 최대 1초를 기다리던 타이머 지연 제거** — 클릭 직후 25ms의
  위치 확정 tick에 첫 metrics 갱신도 함께 예약해 빈 골격이 오래 보이지 않게 함.

## [1.27.0] — 메뉴바 러너와 칩별 CPU 온도 완성도

### Added
- **릴리스 앱의 RunCat 스타일 고양이 러너 복구** — 테스트 전용이던 4프레임
  캐릭터를 실제 메뉴바 렌더링 경로에 연결하고 CPU 사용량에 따라 프레임 속도와
  색상이 바뀌도록 함. 숫자/고양이/둘 다 표시 설정도 실제 출력에 반영함.
- **Apple Silicon M1-M5 세대별 CPU thermal-zone 맵 추가** — 각 칩 세대에
  맞는 SMC 코어 키만 평균에 포함하고, 개별 코어 센서를 전체 센서 목록에 표시함.
- **팬 제어 입력 온도 상태 추가** — 팬 화면과 Settings의 Health 카드에서
  커브 입력 CPU 평균, 코어 최고, 안전 최고, 임계 온도를 서로 구분해 표시함.
- **쓰기 없는 팬 제어 진단과 최근 이력 추가** — Settings에서 팬·온도·
  데몬 상태를 제어 값 변경 없이 점검하고, 최근 제어/실패 12건을 로컬 JSON에
  보관해 문제 원인을 앱 안에서 확인할 수 있게 함.

### Changed
- **CPU Core Average 정책을 칩별 mapped-core 산술 평균으로 통일** — M3용 키를
  모든 Mac에 적용하던 경로를 제거하고, 알 수 없는 칩/Intel에서는 잘못된 평균을
  만들지 않고 aggregate 또는 IOHID 진단값으로 폴백함.
- **유휴 센서 폴링과 러너 렌더링 경량화** — 온도는 2초 캐시를 사용하고,
  고양이를 표시하지 않을 때 애니메이션 타이머를 깨우지 않으며 렌더 프레임을
  미리 생성함. 닫힌 상태에서는 CPU/메모리만 빠르게 갱신하고 프로세스·디스크·
  네트워크는 대시보드가 열렸을 때만 느린 주기로 갱신해 유휴 CPU 사용량을 낮춤.
- **우클릭 메뉴 단순화** — CPU 평균으로 고정된 상단 표시와 모순되던 예전
  CPU/메모리/팬/네트워크 선택 메뉴를 제거하고, 온도 소스는 대시보드 기준임을
  명확히 함. 메뉴 생성 중 동기식 팬 센서 읽기도 제거해 시작 지연을 줄임.

### Fixed
- **실행 중인 오래된 데몬을 `fully ready`로 표시하던 진단 오류 수정** — 최소
  호환 버전보다 낮으면 명시적으로 1회 업데이트가 필요하다고 안내함.
- **소스 설치 문서와 CI 검증 수정** — 실제로 존재하지 않는
  `target/release/PeterFan.app` 경로와 잘못된 Cargo 패키지명을 바로잡고,
  Clippy 경고를 모두 오류로 처리해도 전체 워크스페이스가 통과하도록 정리함.
...
### Fixed
- **상단 아이콘 클릭 시 즉시 무거운 갱신을 실행하지 않도록 변경** —
  팝오버를 먼저 표시하고, 전체 센서 스캔/데몬 IPC는 짧게 지연해 event loop
  멈춤과 클릭 직후 버벅임을 줄임.
- **팝오버 WebView를 유휴 시간에 미리 준비** — 실행 직후에는 조용히 있다가
  약 1.2초 뒤 숨은 WebView를 준비해 첫 클릭의 로딩 부담을 줄임.
- **전체 온도 센서 목록과 데몬 상태 캐시 적용** — 매 tick 전체 SMC 센서
  열람과 IPC를 반복하지 않고, 각각 필요한 주기로 갱신하게 함.
...
### Fixed
- **시작 시 숨은 WebView 생성을 지연** — 앱 실행 직후에는 메뉴바 아이콘만
  먼저 띄우고, 대시보드 WebView는 사용자가 처음 열 때 생성하도록 바꿔
  로그인 자동 실행과 수동 실행의 초기 로딩 부담을 줄임.
- **닫힌 상태의 센서 I/O 축소** — 팝오버가 닫혀 있고 메뉴바가 CPU/메모리 등
  가벼운 지표를 표시할 때는 매초 온도/팬 SMC 호출을 피하도록 해 백그라운드
  버벅임을 줄임.
...
### Fixed
- **오른쪽 버튼 전환 시 팝오버가 출렁이던 문제 완화** — 상태/팬/업데이트/설정/더보기
  화면의 내용 높이가 바뀌어도 네이티브 팝오버 창은 520px 고정 높이를 유지하고,
  넘치는 내용만 왼쪽 패널 안에서 스크롤되도록 정리함.
- **오래된 높이 IPC 호환 처리** — 기존 WebView의 `h:*` 메시지는 유지하되 창 리사이즈
  입력으로 사용하지 않게 해, 팬 카드나 업데이트 문구가 바뀔 때 창 크기가 다시 바뀌지
  않도록 함.
...
### Fixed
- **외부 모니터에서 메뉴바 아이콘을 눌렀을 때 팝오버가 다른 화면에 뜨던 문제** —
  트레이 아이콘 클릭 좌표가 속한 모니터를 찾아 그 화면 안으로 팝오버 위치를
  고정하도록 수정함.
- **위/아래로 배치한 모니터에서 팝오버 높이 계산이 어긋날 수 있던 문제** —
  화면의 y 원점을 반영해, 클릭한 모니터의 하단을 기준으로 높이를 제한하도록 수정함.
...
### Fixed
- **고양이 캐릭터가 RunCat처럼 뛰지 않던 문제** — 기존에는 시스템 지표 갱신과 같은
  1초 틱으로만 메뉴바 아이콘이 바뀌어 움직임이 느리게 보였음.

### Changed
- **메뉴바 고양이 애니메이션 틱 분리** — 센서 샘플링은 1초 주기로 유지하되,
  고양이 아이콘만 별도 70~340ms 틱으로 갱신하도록 바꿈.
- **CPU 사용량 기반 속도 조절** — CPU 사용률이 높을수록 프레임 간격이 짧아져
  더 빠르게 뛰고, 낮을수록 천천히 움직이도록 조정함.

## [1.26.51] — 바로 쓰는 고양이 메뉴바 UX

### Fixed
- **팬 제어가 라이선스 상태로 막히던 문제 제거** — 메뉴바 앱과 데몬의 fanhold/커브
  저장 흐름에서 체험판/라이선스 게이트를 제거해, 설치 후 바로 팬 제어를 사용할 수
  있게 함.
- **전면 License/Activate UI 제거** — 팝오버와 오른쪽 레일에서 라이선스 입력 화면,
  Buy/Activate 진입점을 제거해 버튼 구성을 단순화함.

### Changed
- **RunCat식 고양이 메뉴바 캐릭터** — 기존 사람 형태 러너 아이콘을 고양이 실루엣
  메뉴바 캐릭터로 바꾸고, CPU 부하와 프레임에 따라 색상/다리/꼬리 움직임이
  달라지도록 개선함.
- **오른쪽 레일 단순화** — `Status / Fan / Update / Settings / More` 5개 버튼만
  남겨 모든 버튼이 명확한 화면 전환 역할을 하도록 정리함.

## [1.26.50] — 메뉴바 러너 캐릭터 위치 수정

### Fixed
- **러너 캐릭터 위치 정정** — RunCat처럼 캐릭터가 메뉴바 위에서 보여야 하는데
  팝오버 내부 상단에 표시되던 UI를 제거함.

### Changed
- **메뉴바 아이콘을 CPU 연동 러너로 교체** — 숫자/그래프/둘 다 표시 모드에서
  메뉴바 아이콘 자체가 CPU 부하에 따라 색상과 프레임이 바뀌는 작은 캐릭터로
  동작하도록 변경함.
...
### Changed
- **화면 전환 시 본문 위치 리셋** — 오른쪽 레일 버튼으로 화면을 바꿀 때 왼쪽 본문
  스크롤을 맨 위로 되돌려, 이전 화면의 스크롤 위치 때문에 새 화면이 중간부터
  보이는 어색함을 제거함.
- **레일 버튼 pressed 상태 추가** — 오른쪽 레일 버튼에 `aria-pressed` 상태를
  연결해 현재 선택된 화면을 시각 상태와 접근성 상태 모두에서 일관되게 표시함.

## [1.26.45] — 레일 패널 상태 UI 개선

### Changed
- **전용 화면 상태 pill 추가** — 업데이트/자동 실행/라이선스/더보기 화면 상단에
  현재 상태를 보여주는 pill을 추가해, 버튼을 눌렀을 때 무엇이 켜져 있고 어떤
  상태인지 바로 읽을 수 있게 함.
- **Status 레일 라벨 고정** — 실시간 갱신 중 첫 번째 레일 버튼이 다시 `Detail`로
  바뀌던 흐름을 제거하고, 요약 화면으로 돌아가는 `Status` 역할을 유지함.
- **더보기 화면 한국어 문구 보강** — 새 더보기 설명 문구도 한국어 UI에서 자연스럽게
  보이도록 번역을 추가함.
...
### Changed
- **레일 버튼 역할 통일** — 오른쪽 레일의 `Status`/팬/업데이트/자동 실행/라이선스/
  더보기 버튼을 모두 왼쪽 화면 전환으로 통일함.
- **더보기 명령 화면 추가** — 상세 창 열기와 PeterFan 종료 같은 즉시 실행 명령은
  `더보기` 전용 화면 안의 버튼으로 이동해, 레일 클릭이 갑자기 새 창을 띄우거나
  긴 목록을 펼치지 않게 함.
- **오래된 펼침 상태 무시** — 이전 빌드에서 저장된 확장 상태 때문에 `More`가
  `Less`처럼 보이던 흐름을 제거하고, 고정 높이 팝오버 안에서 항상 같은 조작 모델을
  유지함.

## [1.26.42] — 오른쪽 레일 고정 화면 핫픽스

### Changed
- **메뉴바 팝오버 컴팩트 모드** — 기본 화면은 CPU/메모리/온도/팬 제어에 집중하고,
  저장공간·배터리·네트워크·프로세스·라이선스·종료 같은 보조 섹션은 오른쪽
  `More` 버튼으로 펼치도록 정리함.
- **팝오버 스크롤 영역 정리** — 오른쪽 액션 레일은 고정하고, 스크롤바는 왼쪽
  대시보드 본문 안에 표시되도록 조정함.
...
- **팝오버 레이아웃/레일 UX 개선** — 왼쪽 대시보드 폭을 넓히고, 오른쪽 버튼
  ...
  정리함.
- **팝오버 스크롤바 숨김** — 왼쪽 본문은 계속 스크롤되지만 시각적인 스크롤바는
  숨겨 더 깔끔한 메뉴바 패널처럼 보이게 함.
- **오른쪽 레일 화면 전환** — 팬/업데이트/자동 실행/라이선스/더보기 버튼이
  왼쪽 대시보드의 전용 화면을 전환하도록 바꿔, 라이선스 버튼이 아래 입력폼만
  펼치던 동작을 제거함.
- **팝오버 높이 고정** — 오른쪽 레일 버튼을 누를 때 창 전체가 아래로 늘어나지
  않도록, 창 높이는 고정하고 왼쪽 본문만 전환/스크롤하도록 바꿈.
- **레일 버튼 기능 연결** — 업데이트 버튼은 업데이트 확인을 바로 시작하고,
  자동 실행 버튼은 로그인 항목 토글까지 바로 수행하도록 정리함.

### Fixed
- **오른쪽 레일 전용 패널 표시** — 업데이트/자동 실행/라이선스 전용 화면이
  CSS 기본 숨김 상태에 막혀 보이지 않던 문제를 수정함.

## [1.26.41] — 오른쪽 레일 전용 패널 핫픽스
...
### Changed
- **팝오버를 2열 레이아웃으로 정리** — 왼쪽은 CPU/메모리/저장공간/온도/배터리/
  네트워크 지표를 읽는 영역으로 유지하고, 오른쪽에는 상세 창, 팬 제어,
  업데이트, 자동 실행, 라이선스, 종료를 바로 누르는 큰 액션 버튼 레일을 추가함.
- **하단 명령 버튼을 오른쪽 레일로 흡수** — RunCat처럼 짧은 라벨과 아이콘 중심의
  단순한 조작 모델로 바꿔 첫 화면의 복잡도를 낮춤.
- **팬 제어 바로가기 추가** — 오른쪽 `Fan` 버튼이 팝오버 안의 팬 제어 섹션으로
  바로 이동해 프로파일/팬별 Auto/Manual 조작을 빠르게 찾을 수 있게 함.
...
### Changed
- **런치 시 데몬 업데이트 모달 제거** — 설치된 팬 제어 데몬이 오래된 경우에도
  앱 실행 직후 AppleScript 모달을 띄우지 않고, 상단 Setup 상태와 `Update`
  버튼으로만 안내함. 관리자 암호는 사용자가 직접 업데이트를 누를 때만 요청됨.
- **자동 앱 업데이트 확인과 데몬 업데이트 권한 요청 분리** — 앱 업데이트 확인은
  계속 백그라운드로 실행하지만, root 데몬 교체처럼 권한 상승이 필요한 작업은
  명시적 사용자 액션으로 제한함.

## [1.26.34] — 데몬 업데이트 UX 정리

### Changed
- **호환되는 구버전 데몬 상태를 더 명확히 표시** — 설치된 시스템 데몬이 앱
  버전보다 낮아도 최소 요구 버전 이상이면 Setup 영역에 "정상 / 암호 불필요"로
  보여주어, 앱 업데이트 후 불필요한 관리자 암호 입력을 기대하지 않게 함.
- **데몬 업데이트 버튼 문구 단순화** — 업데이트가 필요한 경우 상단 Setup 버튼이
  내부 구현명인 "Daemon" 대신 "Update"로 보여 사용자가 해야 할 일을 바로
  이해할 수 있게 함.
...
### Changed
- **데몬 상태 문구를 호환성 기준으로 변경** — 앱/데몬 버전이 달라도 설치된
  데몬이 앱의 최소 요구 버전을 만족하면 `호환됨`으로 표시해, 불필요한 데몬
  업데이트와 관리자 암호 입력을 유도하지 않게 함.
- **데몬 업데이트 완료 메시지 정확화** — 데몬 설치/업데이트 완료 알림이 앱
  버전을 가정하지 않고, 실제 설치된 `/usr/local/bin/peterfand` 버전을 다시
  읽어 표시하도록 함.

## [1.26.25] — 데몬 업데이트 프롬프트 완화

### Fixed
- **데몬 업데이트 암호 프롬프트 기준 수정** — 앱 버전과 시스템 데몬 버전이 단순히
  다르다는 이유만으로 관리자 암호를 요구하지 않고, 설치된 데몬이 앱이 실제로
  요구하는 최소 데몬 버전보다 낮을 때만 업데이트를 안내하도록 함.
